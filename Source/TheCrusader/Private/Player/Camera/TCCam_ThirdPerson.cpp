...
#include "Player/Camera/TCCam_ThirdPerson.h"
                                      ...
                                      FTViewTarget& OutVT)
{
	...
	if (World)
	{
		...
		PlayerCameraManager->PCOwner->SetControlRotation(PivotToWorld.GetRotation().Rotator());
...
void UTCCam_ThirdPerson::OnBecomeActive(AActor* ViewTarget, UTCCameraMode* PreviouslyActiveMode)
{
...
void UTCCam_ThirdPerson::SkipNextInterpolation()
{
...
FTransform UTCCam_ThirdPerson::GetCameraToPivot(const AActor* ViewTarget) const
{
...
FTransform UTCCam_ThirdPerson::GetViewTargetToWorld(const AActor* ViewTarget) const
{
	return ViewTarget ? ViewTarget->GetActorTransform() : FTransform::Identity;
}

float UTCCam_ThirdPerson::GetViewTargetMeshHeightOffset(const AActor* ViewTarget) const
{
	const ABalian* const Char = Cast<ABalian>(ViewTarget);
	if (Char && PlayerCameraManager)
	{
...
FQuat UTCCam_ThirdPerson::GetAutoFollowPivotToWorldRotation(const AActor* FollowActor) const
{
...
FTransform UTCCam_ThirdPerson::ComputePivotToWorld(const AActor* ViewTarget) const
{
	...
	PivotToWorld.SetRotation(PlayerCameraManager->PCOwner->GetControlRotation().Quaternion());
		...
		if (!bSkipNextInterpolation)
		{
			...
			else
			{
				...
				if (bAllowLazyAutoFollowPitchControl)
				{
					...
					float const ControlPitch = PlayerCameraManager->PCOwner->GetControlRotation().Pitch;
...
FVector UTCCam_ThirdPerson::ComputeWorldLookAtPosition(const FVector IdealWorldLookAt, float DeltaTime)
{
	return LookatWorldSpaceInterpolator.Eval(IdealWorldLookAt, DeltaTime);
}

FRotator UTCCam_ThirdPerson::ComputeSmoothPivotRotation(const FRotator IdealPivotToWorldRot, float DeltaTime)
{
	if (bSkipNextInterpolation)
	{
		PivotRotInterpolator.Reset();
