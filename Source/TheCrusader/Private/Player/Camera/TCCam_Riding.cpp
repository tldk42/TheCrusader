...
#include "Player/Camera/TCCam_Riding.h"
...
void UTCCam_Riding::UpdateCamera(AActor* ViewTarget, UCineCameraComponent* CineCamComp, float DeltaTime,
                                 FTViewTarget& OutVT)
{
	//Obtain all the data we need from the vehicle
	float TimeSpentSteering = 0.0f;
	if (const AHorse_Base* Horse_Base = Cast<AHorse_Base>(ViewTarget))
	{
		DrivingState = Horse_Base->GetRidingState();
	}

	// Update Interpolation Speeds
	const float CurrentSpeed = ViewTarget ? (ViewTarget->GetVelocity().Size()) : 0.0f;

	const float SharpLookAtSpeedAlpha = FMath::Clamp(
		FMath::GetRangePct(DrivingLookatWorldSpaceInterpScaleRange, CurrentSpeed), 0.0f, 1.0f);
	const float SharpLookAtPrimaryInterpSpeed = FMath::Lerp(DrivingLookatWorldSpaceInterpSpeed.X,
	                                                        DrivingLookatWorldSpaceInterpSpeed.X *
	                                                        DrivingLookatWorldSpaceInterpMaxSpeedScale,
	                                                        SharpLookAtSpeedAlpha);
	const float SharpLookAtIntermediateInterpSpeed = FMath::Lerp(DrivingLookatWorldSpaceInterpSpeed.Y,
	                                                             DrivingLookatWorldSpaceInterpSpeed.Y *
	                                                             DrivingLookatWorldSpaceInterpMaxSpeedScale,
	                                                             SharpLookAtSpeedAlpha);
	...
	if (SteeringToLookAtLateralOffset != nullptr && FMath::Abs(DrivingState.Steering) >=
		LookAtLateralOffsetSteeringThreshold)
	{
		const float Direction = DrivingState.Steering > 0 ? 1.f : -1.f;
		const float LateralLookAtOffsetTarget = SteeringToLookAtLateralOffset->GetFloatValue(
				TimeSpentSteering / SteeringToLookAtLateralOffsetScales.X) * SteeringToLookAtLateralOffsetScales.Y *
			Direction;

		LookatOffsetLocal.Y = FMath::FInterpConstantTo(LookatOffsetLocal.Y, LateralLookAtOffsetTarget, DeltaTime,
		                                               LateralLookAtOffsetRiseAndFallSpeeds.X);
	}
	else
	{
		LookatOffsetLocal.Y = FMath::FInterpConstantTo(LookatOffsetLocal.Y, DefaultLateralLookAtOffset, DeltaTime,
		                                               LateralLookAtOffsetRiseAndFallSpeeds.Y);
	}
	...
	else
	{
		...
		AerialCameraToPivotOffsetInterpolator.IntermediateInterpSpeed = AerialCameraToPivotOffsetGroundTransitionSpeeds.
			Y;
...
void UTCCam_Riding::OnBecomeActive(AActor* ViewTarget, UTCCameraMode* PreviouslyActiveMode)
{
	...
	if (const AHorse_Base* Vehicle = Cast<AHorse_Base>(ViewTarget))
	{
...
void UTCCam_Riding::SkipNextInterpolation()
{
...
float UTCCam_Riding::ComputeFinalFOV(const AActor* ViewTarget) const
{
	...
	if (bUseFOVSpeedAdjustmentCurve && FOV_SpeedAdjustmentCurve != nullptr)
	{
		...
		const float CurrentSpeed = ViewTarget ? (ViewTarget->GetVelocity().Size()) : 0.0f;
		const float SpeedAlpha = FMath::Clamp(FMath::GetRangePct(FOV_SpeedAdjustment_SpeedRange, CurrentSpeed), 0.0f,
		                                      1.0f);
...
float UTCCam_Riding::ComputeYawModifier(const AActor* ViewTarget, float DeltaTime)
{
	if (bUseSteeringYawModifiers && SpeedToYawMultiplierCurve != nullptr)
	{
		if (const AHorse_Base* Vehicle = Cast<AHorse_Base>(ViewTarget))
		{
			...
			const float SpeedAlpha = FMath::Clamp(
				FMath::GetRangePct(SpeedToYawMultiplierCurve_SpeedRange, CurrentSpeed), 0.0f, 1.0f);
			float GoalYawModifier = (Vehicle->InputSide * 0.1f * SpeedToYawMultiplierCurve->
				GetFloatValue(SpeedAlpha) * SpeedToYawMultiplierCurve_Scale);

			// Only apply a potential handbrake modifier if the vehicle is experiencing velocity
			if (Vehicle->InputSide != 0 && !ViewTarget->GetVelocity().IsNearlyZero())
			{
				GoalYawModifier += HandbrakeYawModifier * Vehicle->InputSide * 0.1f;
			}
...
float UTCCam_Riding::ComputeRollModifier(const AActor* ViewTarget, float DeltaTime)
{
	if (bUseSteeringRollModifiers && SpeedToRollMultiplierCurve != nullptr)
	{
		if (const AHorse_Base* Vehicle = Cast<AHorse_Base>(ViewTarget))
		{
			...
			const float CurrentSteeringInput = Vehicle->InputSide * 0.1f;
			...
			const float CurrentSpeed = ViewTarget ? (ViewTarget->GetVelocity().Size()) : 0.0f;
			const float SpeedAlpha = FMath::Clamp(
				FMath::GetRangePct(SpeedToRollMultiplierCurve_SpeedRange, CurrentSpeed), 0.0f, 1.0f);
			float GoalRollModifier = (CurrentSteeringInput * SpeedToRollMultiplierCurve->GetFloatValue(SpeedAlpha) *
				SpeedToRollMultiplierCurve_Scale);
...
void UTCCam_Riding::AdjustAutoFollowMode(const AActor* ViewTarget)
{
	...
	if (UGameInstance* GameInstance = UGameplayStatics::GetGameInstance(GetWorld()))
	{
		...
		AutoFollowMode = FMath::Abs(DrivingState.ForwardSpeed) > LazyFollowSpeedThreshold
			                 ? ECameraAutoFollowMode::LazyFollow
			                 : ECameraAutoFollowMode::None;
		...
		else if ((DrivingState.bHandbrakeOn && DrivingState.ForwardSpeed > FowardVelocityThreshold) || (DrivingState.
			ForwardSpeed < KINDA_SMALL_NUMBER && !bReverseFollowActive))
		{
		...
		else if (!bIsDrifting && !DrivingState.bHandbrakeOn && DrivingState.ForwardSpeed > FowardVelocityThreshold && !
			GameInstance->GetTimerManager().IsTimerActive(HandbrakeReleaseTimer))
		{
			LazyFollowLaziness = DrivingLazyFollowLaziness;
...
void UTCCam_Riding::ComputePredictiveLookAtPoint(FVector& LookAtPointOutput, const AActor* ViewTarget, float DeltaTime)
{
	...
	if (LazyFollowDelay_TimeRemaining > 0.0f)
	{
		...
		else
		{
			...
			const float LookAtWorldSpaceInterpAlpha = FMath::Clamp(
				FMath::GetRangePct(FVector2D(0.0f, LookAtManualFollowInterpSpeedTransitionTime),
				                   TimeSpentInManualCameraControl), 0.0f, 1.0f);
			LookatWorldSpaceDoubleInterpolator.PrimaryInterpSpeed = FMath::Lerp(
				LookatWorldSpaceDoubleInterpolator.PrimaryInterpSpeed,
				LookatWorldSpaceDoubleInterpolator.PrimaryInterpSpeed * LookAtManualInterpSpeedTransitionScalar,
				LookAtWorldSpaceInterpAlpha);
			LookatWorldSpaceDoubleInterpolator.IntermediateInterpSpeed = FMath::Lerp(
				LookatWorldSpaceDoubleInterpolator.IntermediateInterpSpeed,
				LookatWorldSpaceDoubleInterpolator.IntermediateInterpSpeed * LookAtManualInterpSpeedTransitionScalar,
				LookAtWorldSpaceInterpAlpha);
		}
	...
	else
	{
		...
		const bool bInTransitionFromManualCamera = TimeSpentInAutomaticCameraControl <
			LookAtLazyFollowInterpSpeedTransitionTime;
		if (bInTransitionFromManualCamera)
		{
			const float LookAtWorldSpaceInterpAlpha = FMath::Clamp(
				FMath::GetRangePct(FVector2D(0.0f, LookAtLazyFollowInterpSpeedTransitionTime),
				                   TimeSpentInAutomaticCameraControl), 0.0f, 1.0f);
			LookatWorldSpaceDoubleInterpolator.PrimaryInterpSpeed = FMath::Lerp(
				LookatWorldSpaceDoubleInterpolator.PrimaryInterpSpeed * LookAtManualInterpSpeedTransitionScalar,
				LookatWorldSpaceDoubleInterpolator.PrimaryInterpSpeed, LookAtWorldSpaceInterpAlpha);
			LookatWorldSpaceDoubleInterpolator.IntermediateInterpSpeed = FMath::Lerp(
				LookatWorldSpaceDoubleInterpolator.IntermediateInterpSpeed * LookAtManualInterpSpeedTransitionScalar,
				LookatWorldSpaceDoubleInterpolator.IntermediateInterpSpeed, LookAtWorldSpaceInterpAlpha);
		}
		...
		if (bReverseFollowActive)
		{
			...
			else
			{
				...
				VectorMultiplier = MinLookAtPointDistance - (FMath::Min(SteeringReverseOffset, CameraAngleReverseOffset)
					* ThrottleMultiplier);
			}
		}
	...
	LookAtPointOutput = LookAtPointOutput + ViewTarget->GetActorForwardVector() * FinalVectorMultiplier *
		PredictiveLookatTime;
}

FVector UTCCam_Riding::ComputeWorldLookAtPosition(const FVector IdealWorldLookAt, float DeltaTime)
{
	return LookatWorldSpaceDoubleInterpolator.Eval(IdealWorldLookAt, DeltaTime);
}

FRotator UTCCam_Riding::ComputeSmoothPivotRotation(const FRotator IdealPivotToWorldRot, float DeltaTime)
{
	...
	if (CVarDrawDrivingCamDebug.GetValueOnGameThread())
	{
		...
		const FString DisplayString5 = FString::Printf(
			TEXT("Primary Rot Interp Speed: %f,  Intermediate Rot Interp Speed: %f"),
			PivotRotInterpolator.PrimaryInterpSpeed, PivotRotInterpolator.IntermediateInterpSpeed);
...
void UTCCam_Riding::UpdatePivotRotInterpSpeeds(const FRotator& GoalRotation, float DeltaTime)
{
	...
	FVector2D GoalInterpSpeeds = FVector2D(PivotRotInterpolator.PrimaryInterpSpeed,
	                                       PivotRotInterpolator.IntermediateInterpSpeed);
	...
	if (!bTransitionBetweenForwardAndReverseActive && !bTransitionBetweenManualToAutoCameraActive)
	{
		...
		{
			GoalInterpSpeeds = FVector2D(DrivingFullFollowPivotRotInterpSpeed.X,
			                             DrivingFullFollowPivotRotInterpSpeed.Y);
		}
		else
		{
	...
	if (bExitingTransitionalPivotRotInterpSpeeds)
	{
		...
		const float PivotRotInterpSpeedAlpha = FMath::Clamp(
			FMath::GetRangePct(FVector2D(0, PivotRotSpeedChangeTimeToTransition),
			                   PivotRotSpeedChangePassedTransitionTime), 0.0f, 1.0f);
		PivotRotInterpolator.PrimaryInterpSpeed = FMath::Lerp(ActiveTransitionalPivotRotInterpSpeeds.X,
		                                                      GoalInterpSpeeds.X, PivotRotInterpSpeedAlpha);
		PivotRotInterpolator.IntermediateInterpSpeed = FMath::Lerp(ActiveTransitionalPivotRotInterpSpeeds.Y,
		                                                           GoalInterpSpeeds.Y, PivotRotInterpSpeedAlpha);
...
void UTCCam_Riding::CheckForRotationalTransitionTriggers(FVector2D& GoalPivotRotInterpSpeeds)
{
...
void UTCCam_Riding::TriggerTransitionBetweenForwardAndReverseBehavior(FVector2D& GoalPivotRotInterpSpeeds)
{
...
void UTCCam_Riding::TriggerTransitionBetweenManualAndAutoBehavior(FVector2D& GoalPivotRotInterpSpeeds)
{
